...
public class EmployeeController {
	...
	// 2. 當 empId 合法，但是因為網路或資料庫忙碌，而發生問題
	...
	public Employee getEmployee4(@PathVariable Integer empId) throws InterruptedException {
		...
		// 模擬業務處理遞延
		// 給 @Bulkhead 使用
		// 當執行 http://localhost:6060/employee/semaphone/1 過多時就會印出 "Bulkhead call
		// Rejected"
		Thread.sleep(3000);
	...
	public CompletableFuture<Employee> getEmployee5(@PathVariable Integer empId) {
		return CompletableFuture.supplyAsync(() -> {
			...
			// 模擬業務處理遞延
			// 給 @Bulkhead 使用
			// 當執行 http://localhost:6060/employee/threadpool/1 過多時就會印出 "Bulkhead call
			// Rejected"
			try {
				Thread.sleep(3000);
	...
	// 6. 利用 TimeLimiter 處理錯誤，目的是限制方法執行的最大時間，防止長時間未響應的請求拖垮系統
